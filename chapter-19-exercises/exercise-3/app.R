...
#setwd("~/Desktop/book-exercises/chapter-19-exercises/exercise-3/")
...
# (of your sample)
prince_range <- (diamonds_sample$price)
...
my_ui <- fluidPage(
  ...
  titlePanel("Diamond View"),
  h1("I am at the top"),
  p("This is a chart of diamonds"),
  a("Link to the type of fishes", herf ="https://fishingbooker.com/fish"),
  price_input,
  feature_input,
...
my_server <- function(input_list, output_list) {
  ...
  output_list$plot <- renderPlot({
    ...
    filter_data <- diamonds_sample %>% 
      filter(price > input_list$price_choice[1], price < input_list$price_choice[2])
